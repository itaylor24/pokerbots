...
if __name__ == '__main__':
    ...
    print('done w card combos')

    suited_strengths = [calc_strength([hole[0] + 'c', hole[1] + 'c'], _MONTE_CARLO_ITERS) for hole in off_rank_holes] #all holes with the same suit
    off_suit_strengths = [calc_strength([hole[0] + 'c', hole[1] + 'd'], _MONTE_CARLO_ITERS) for hole in off_rank_holes] #all holes with off suits
    pocket_pair_strengths = [calc_strength([hole[0] + 'c', hole[1] + 'd'], _MONTE_CARLO_ITERS) for hole in pocket_pair_holes] #pocket pairs must be off suit
    print('done calculating strengths')
    ...
    hole_df.to_csv('old_hole_strengths.csv', index=False) #save it for later use, trade space for time!
